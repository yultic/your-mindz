...
export async function POST(req: NextRequest) {
  try {
    ...
    // resilencia-Networkfetch  PayPal API
    try {
          ...
          purchase_units: [
            {
              amount: {
                ...
                // Important: breakdown is required if items are present
                breakdown: {
                  item_total: {
                    currency_code: body.currency ?? 'USD',
                    value: parsedAmount.toFixed(2),
              ...
              items: [
                {
                  name: body.description.substring(0, 127), // PayPal limite a 127 chars
                  quantity: "1",
                  unit_amount: {
                    currency_code: body.currency ?? 'USD',
                    value: parsedAmount.toFixed(2),
              ...
              note_to_payer: body.description,
              // custom_id es un campo opcional, no se muestra al cliente
      ...
      console.error('[PayPal] Network error while calling PayPal:', networkErr)
      return NextResponse.json(
        { error: 'PayPal service currently unavailable (Network Error)' },
        { status: 503 }
      )
    }
  } catch (err) {
